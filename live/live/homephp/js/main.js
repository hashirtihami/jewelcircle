(function ($) {
    ...
    $('.js-show-modal1').on('click',function(e){
        ...
        $.post("templates/modal.inc.php", {title: title}, function(data){
            ...
            if(parseInt(DATA.category)){
            console.log("hi");
                $.post('checkCat.php', {category: DATA.category}, function(data){
                console.log(data);
                    var DATA = JSON.parse(data);
                    if(DATA.category == 'Zodiac'){
                        $("input[name='nameOnProduct']").remove();
                        $("#nameInput").html('<select class="js-select2" name="nameOnProduct"><option>Aries</option><option>Leo</option><option>Sagittarius</option><option>Taurus</option><option>Virgo</option><option>Capricorn</option><option>Gemini</option><option>Libra</option><option>Aquarius</option><option>Cancer</option><option>Scorpio</option><option>Pisces</option></select>');
                    }
                })
            }
            if(DATA.size === 'TRUE'){
